/*
  ...
  1. Yeni Tablolar
    - `likes`
      - `id` (uuid, primary key)
      - `user_id` (uuid, referanslar profiles.id)
      - `liked_user_id` (uuid, referanslar profiles.id)
      - `created_at` (timestamp)
    
    - `profile_views`
      - `id` (uuid, primary key)
      - `viewer_id` (uuid, referanslar profiles.id)
      - `viewed_user_id` (uuid, referanslar profiles.id)
      - `created_at` (timestamp)
...
CREATE TABLE IF NOT EXISTS profile_views (
  ...
  created_at timestamptz DEFAULT now(),
  UNIQUE(viewer_id, viewed_user_id, created_at::date)
);
